...
class ScannerService:
    ...
    def scan_image(
        ...
        try:
            ...
            logger.info(
                f"Scan completed for {image_name}:{image_tag} - "
    ...
    def _update_scan_record(
        self, scan_record: VulnerabilityScan, scan_result: ScanResult
    ):
    ...
    def _apply_exceptions(self, scan_record: VulnerabilityScan):
        ...
        if excepted_cves:
            ...
            logger.info(
                f"Applied {len(excepted_cves)} exceptions to scan {scan_record.id}"
            )
    ...
    def _update_vulnerability_counts(self, scan_record: VulnerabilityScan):
        ...
        counts = (
            ...
            .filter(Vulnerability.scan_id == scan_record.id)
    ...
    def should_fail_build(
        self, scan_record: VulnerabilityScan, threshold: str = "HIGH"
    ) -> bool:
    ...
    def get_scan_statistics(self, days: int = 30) -> Dict[str, Any]:
        ...
        top_vulnerable_images = sorted(
            image_stats.items(), key=lambda x: x[1]["total_vulns"], reverse=True
        )[:10]
    ...
    def _send_scan_notifications(self, scan_record: VulnerabilityScan):
        ...
        try:
            ...
            configs = NotificationMapper.get_configurations_by_group(
                scan_record.group_id
            )

            if not configs:
                logger.info(
                    f"No notification configurations found for group {scan_record.group_id}"
                )
                return
            ...
            scan_data = {
                ...
                "scan_timestamp": scan_record.scan_timestamp.isoformat()
                if scan_record.scan_timestamp
                else None,
            }
            ...
            for config in configs:
                try:
                    ...
                    logger.error(
                        f"Failed to send notification via {config.service}: {e}"
                    )
    ...
    def _send_slack_notification(self, config, scan_data):
        ...
        try:
            ...
            slack_service = create_slack_service(
                webhook_url=config.webhook_url, channel=config.channel or "#general"
            )
