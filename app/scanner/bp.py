...
def scan_image():
    ...
    try:
        ...
        if scanner_type not in ["trivy", "clair"]:
            return (
                jsonify({"error": "Invalid scanner type. Must be trivy or clair"}),
                400,
            )
        ...
        config = {
            ...
            "timeout": current_app.config.get("SCANNER_TIMEOUT", 300),
            "severity_threshold": current_app.config.get(
        ...
        if not group_id:
            return (
                jsonify({"error": "No group selected. Please select a group first."}),
                400,
            )
    ...
    try:
        ...
        if severity_filter:
            vulnerabilities = [
                ...
                if v.severity.upper() == severity_filter.upper()
            ]
...
def get_scan_history():
    ...
    try:
        ...
        return (
            jsonify({"scans": [scan.to_dict() for scan in scans], "count": len(scans)}),
            200,
        )
