...
class ScanMapper:
    ...
    def get_scans_by_group(group_id, limit=SystemConstants.DEFAULT_SCAN_LIMIT):
        return VulnerabilityScan.query.filter(
            VulnerabilityScan.group_id == group_id
        ).order_by(VulnerabilityScan.created_at.desc()).limit(limit).all()
    
    @staticmethod
    def get_scans_by_image(image_name, group_id=None, limit=SystemConstants.DEFAULT_SCAN_LIMIT):
    ...
    def get_recent_scans(group_id, limit=SystemConstants.DEFAULT_RECENT_SCANS_LIMIT):
        return VulnerabilityScan.query.filter(
            VulnerabilityScan.group_id == group_id
        ).order_by(VulnerabilityScan.scan_timestamp.desc()).limit(limit).all()
    ...
    def get_scan_statistics(group_id, days=SystemConstants.DEFAULT_STATISTICS_DAYS):
        ...
        successful_scans = len([s for s in scans if s.scan_status == DatabaseConstants.SCAN_STATUSES[0]])
