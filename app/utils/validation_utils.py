...
class SeverityUtils:
    ...
    def should_fail_build(
        ...
        medium_count: int,
        low_count: int,
        threshold: str = DatabaseConstants.DEFAULT_VULNERABILITY_THRESHOLD,
    ) -> bool:
...
class ResponseUtils:
    ...
    def success_response(
        data: Any = None, message: str = SuccessMessages.SUCCESS
    ) -> Dict[str, Any]:
    ...
    @staticmethod
    def error_response(
        message: str, error_code: int = HTTPStatusCodes.BAD_REQUEST
    ) -> Dict[str, Any]:
        return {
            ResponseKeys.SUCCESS: False,
            ResponseKeys.ERROR: message,
            ResponseKeys.ERROR_CODE: error_code,
        }
