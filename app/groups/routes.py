from flask import request, jsonify, render_template, session, current_app, flash, redirect, url_for
...
def leave_group(group_id):
    try:
        ...
        if user_role == 'owner':
            return jsonify({'error': 'Group owners cannot leave the group. Transfer ownership first.'}), 400
    ...
    try:
        ...
        for member in group.members:
            ...
            members_data.append({
                'id': member.id,
                'email': member.email,
                'name': f"{member.first_name} {member.last_name}".strip() if member.first_name or member.last_name else member.email,
...
def update_member_role(group_id, member_id):
    try:
        ...
        if user_role not in ['admin', 'owner']:
            return jsonify({'error': 'You do not have permission to manage this group'}), 403
        ...
        if new_role == 'admin' and user_role != 'owner':
            return jsonify({'error': 'Only group owners can promote members to admin'}), 403
