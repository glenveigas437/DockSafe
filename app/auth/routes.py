...
def login():
    if not GOOGLE_OAUTH_AVAILABLE or not GoogleOAuthService or not GoogleOAuthService.is_configured():
...
def google_callback():
    if not GOOGLE_OAUTH_AVAILABLE or not GoogleOAuthService:
        flash('Google OAuth is not available. Please contact your administrator.', 'error')
        return redirect(url_for('auth.login'))
    ...
    try:
        ...
        current_app.logger.info(f"OAuth callback received - Code: {authorization_code[:10] if authorization_code else 'None'}..., State: {state}")
        current_app.logger.info(f"Session ID in callback: {session.get('_id', 'unknown')}")
        ...
        if state != stored_state:
            current_app.logger.warning(f"State mismatch: stored={stored_state}, received={state}")
