...
from flask import render_template, current_app, jsonify, redirect, url_for, session, request
...
def index():
        ...
        if user:
            for group in user.groups:
                user_groups.append({
                    ...
                    'scan_count': len(list(group.vulnerability_scans)) if hasattr(group, 'vulnerability_scans') else 0
                })
    ...
    show_group_modal = (request.args.get('show_group_modal') == 'true' and 
                        len(user_groups) > 1 and 
