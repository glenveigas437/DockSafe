...
class VulnerabilityScan(db.Model):
    ...
    scan_status = db.Column(db.String(50), nullable=False)  # SUCCESS, FAILED, IN_PROGRESS
...
class ScanException(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    cve_id = db.Column(db.String(50), nullable=False, index=True)
    image_name = db.Column(db.String(255), nullable=True, index=True)  # Null for global exceptions
...
class ScanConfiguration(db.Model):
    ...
    description = db.Column(db.Text)
    scanner_type = db.Column(db.String(20), default='trivy')  # trivy, clair
    vulnerability_threshold = db.Column(db.String(20), default='HIGH')  # CRITICAL, HIGH, MEDIUM, LOW
