...
class NotificationManager:
            ...
            for config in configs:
                if config.type == 'chat' and config.service == 'Slack':
                    ...
                    email_service = create_email_service(
                        smtp_server=config.additional_config.get('smtp_server', 'smtp.gmail.com'),
    ...
    def send_vulnerability_alert(self, scan_data: Dict) -> Dict[str, List[Dict]]:
        ...
        for slack_service in self.slack_services:
                ...
                logger.info(f"Slack vulnerability alert ({slack_service.config.name}): {'sent' if success else 'failed'}")
            except Exception as e:
        ...
        for email_service in self.email_services:
                ...
                logger.info(f"Email vulnerability alert ({email_service.config.name}): {'sent' if success else 'failed'}")
            except Exception as e:
    ...
    def send_scan_completion_notification(self, scan_data: Dict) -> Dict[str, List[Dict]]:
        results = {
            'slack': [],
            'teams': [],
            'email': []
        ...
        for slack_service in self.slack_services:
                ...
                logger.info(f"Slack scan completion notification ({slack_service.config.name}): {'sent' if success else 'failed'}")
            except Exception as e:
                logger.error(f"Error sending Slack scan completion notification: {str(e)}")
                results['slack'].append({
                    'config_name': getattr(slack_service.config, 'name', 'Unknown'),
                    'success': False,
                    'message': f'Error: {str(e)}'
        ...
        for email_service in self.email_services:
                ...
                logger.info(f"Email scan completion notification ({email_service.config.name}): {'sent' if success else 'failed'}")
            except Exception as e:
                logger.error(f"Error sending Email scan completion notification: {str(e)}")
                results['email'].append({
                    'config_name': getattr(email_service.config, 'name', 'Unknown'),
                    'success': False,
                    'message': f'Error: {str(e)}'
    ...
    def send_test_message(self) -> Dict[str, List[Dict]]:
        ...
        for slack_service in self.slack_services:
                ...
                logger.info(f"Slack test message ({slack_service.config.name}): {'sent' if success else 'failed'}")
            except Exception as e:
        ...
        for email_service in self.email_services:
                ...
                logger.info(f"Email test message ({email_service.config.name}): {'sent' if success else 'failed'}")
            except Exception as e:
