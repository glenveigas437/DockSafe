...
class NotificationService:
        ...
        for config in configs:
            if config.type == "chat" and config.service == "Slack":
                ...
                if config.additional_config:
                    settings_data["email_smtp_server"] = config.additional_config.get(
                        "smtp_server", "smtp.gmail.com"
                    )
                    settings_data["email_smtp_port"] = config.additional_config.get(
                        "smtp_port", 587
                    )
                    settings_data["email_username"] = config.additional_config.get(
                        "username", ""
                    )
                    settings_data["email_password"] = config.additional_config.get(
                        "password", ""
                    )
    ...
    def save_notification_settings(group_id, data):
        if data.get("slack_webhook"):
            existing_slack = NotificationMapper.get_configuration_by_type_service(
                "chat", "Slack", group_id
            )

            if existing_slack:
                NotificationMapper.update_configuration(
                    existing_slack.id,
        ...
        if data.get("teams_webhook"):
            existing_teams = NotificationMapper.get_configuration_by_type_service(
                "chat", "Teams", group_id
            )
        ...
        if data.get("email_recipients"):
            existing_email = NotificationMapper.get_configuration_by_type_service(
                "email", "Gmail", group_id
            )
    ...
    def test_notifications(group_id, test_type="all"):
        ...
        if test_type == "all" or test_type == "slack":
            ...
            slack_configs = [
                c for c in slack_configs if c.type == "chat" and c.service == "Slack"
            ]
        ...
        if test_type == "all" or test_type == "teams":
            ...
            teams_configs = [
                c for c in teams_configs if c.type == "chat" and c.service == "Teams"
            ]
