...
class ScanService:
    def __init__(
        self, scanner_type=DatabaseConstants.DEFAULT_SCANNER_TYPE, config=None
    ):
    ...
    def _apply_exceptions(self, scan_id, image_name):
        ...
        if excepted_cves:
            VulnerabilityMapper.delete_vulnerabilities_by_cve_ids(
                scan_id, excepted_cves
            )
    ...
    def should_fail_build(
        self, scan_record, threshold=DatabaseConstants.DEFAULT_VULNERABILITY_THRESHOLD
    ):
