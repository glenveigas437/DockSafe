...
def setup_email_config():
    ...
    if os.path.exists(env_file):
        ...
        overwrite = input("Do you want to update the email configuration? (y/N): ").lower()
        if overwrite != 'y':
    ...
    if choice == '1':  # Gmail
        ...
        config.update({
            ...
            'DOCSAFE_EMAIL_SMTP_PORT': input("Enter SMTP port (default 587): ") or '587',
            'DOCSAFE_EMAIL_USERNAME': input("Enter SMTP username: "),
            'DOCSAFE_EMAIL_FROM': input("Enter sender email: "),
            'DOCSAFE_EMAIL_USE_TLS': input("Use TLS? (y/N): ").lower() == 'y' and 'true' or 'false'
        })
